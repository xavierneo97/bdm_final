...
def main(sc):
  ...
  # Get total distance and total visitor numbers per CBG for each date, and calculate average distance
  df6 = df5.groupBy('cbg_fips','date') \
  .sum('visitor_count','dist') \
  .cache()

  # Calculate avg distance by dividing total distance by total visitors
  df7 = df6.withColumn('avg_dist', F.round(df6[3]/df6[2], 2)) \
    .cache()
  ...
  # Pivot df by date
  # Sort by cbg_fips
  pivotdf = df7.groupBy('cbg_fips').pivot('date').max('avg_dist') \
    .sort('cbg_fips') \
    .cache()

  # Replace null with blanks
  pivotdf = pivotdf.select( *[ F.when(F.col(column).isNull(),'').otherwise(F.col(column)).alias(column) for column in pivotdf.columns]) \
    .cache()  

  # SAVE AS CSV
  import itertools
  output_rdd = pivotdf.rdd.map(lambda x: (x[0],x[1],x[2],x[3],x[4])).cache()
