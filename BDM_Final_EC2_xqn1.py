...
def main(sc):
  ...
  # EXTRA CREDIT 2
  # STEP 1: FILTER MAIN DATAFRAME FOR VISITS TO NYC SUPERMARKETS USING core-places-nyc.csv

  # Read core-places-nyc and filter for places with naics_code starting with 4451
  df_supermarkets = spark.read.csv('/tmp/bdm/core-places-nyc.csv', header=True) \
        .select(F.col('placekey').alias('naics_placekey'),
        ...
        .na.drop() \
        .where(F.col('naics_code').startswith('4451')) \
        .cache()

  set_supermarkets = set(df_supermarkets.select('naics_placekey').distinct().rdd.flatMap(lambda x: x).collect())
  ...
  dist_df = df3.withColumn('origin-dest', merge_od(F.col('placekey'), F.col('key'))) \
    ...
    .distinct() \
    .join(df_supermarkets, F.col('placekey')==df_supermarkets['naics_placekey'], how='inner') \
  ...
  df4 = df3.withColumn('origin-dest', merge_od(F.col('placekey'), F.col('key'))) \
    ...
    .csv(sys.argv[1] if len(sys.argv)>1 else 'output_ec2')
